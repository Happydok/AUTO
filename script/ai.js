...
module.exports.run = async function({ api, event, args }) {
  ...
  if (input === "clear") {
    try {
      await axios.post('https://deku-rest-api.gleeze.com/gpt4?prompt=hi&uid=100', { id: event.senderID });
  ...
  try {
    ...
    const { data } = await axios.post('https://deku-rest-api.gleeze.com/gemini?', {
      prompt: input,
